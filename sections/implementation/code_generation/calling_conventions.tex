...
This analysis assumes that the args are ready and can be moved or pushed directly, however in some cases, such as offset loading/storing, the arg first needs to be computed. If the destination is a register, as in the case of the first 2 \code{\_\_fastcall} arguments, then the move can potentially be avoided by using the destination register as the accumulator for the calculation. In the case of a stack destination, such as in \code{\_\_stdcall}, this optimisation cannot be made.
